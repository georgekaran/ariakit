...
import { getRequestedState, subscribe } from "@ariakit/store";
import {
  flatten2DArray,
  reverseArray,
  afterPaint,
...
const CompositeFocusOnMove = memo(function CompositeFocusOnMove({
  ...
  useEffect(() => {
    ...
    unsubscribe = subscribe(store, ["activeId", "items"], () => {
      // Compared against the requested value so a controlled activeId that
      // hasn't committed yet doesn't read as a superseding move.
      if (getRequestedState(store, "activeId") !== activeId) {
        cleanup();
        return;
      }
      const itemElement = getEnabledItem(store, activeId)?.element;
